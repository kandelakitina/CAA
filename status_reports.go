...
func (app *application) loadStatusReport(c *gin.Context) ([]statusReportRow, error) {
	...
	for rows.Next() {
		var item statusReportRow
		var is, io, cs, co string
		if err := rows.Scan(&item.ID, &item.Title, &item.Status, &item.Archived, &is, &io, &item.InternalDeadline, &item.InternalTotal, &item.InternalDone, &item.InternalPending, &cs, &co, &item.CommitteeDeadline, &item.CommitteeTotal, &item.CommitteeDone, &item.CommitteePending, &item.Overdue); err != nil {
			return nil, err
		}
		item.Status = questionStatusLabel(item.Status)
		item.Internal = reportRoundLabel(is, io)
		item.Committee = reportRoundLabel(cs, co)
		for _, service := range requiredInternalServices {
			item.InternalPending = strings.ReplaceAll(item.InternalPending, service, internalServiceLabel(service))
		}
		if is != "active" {
			item.InternalPending = ""
		}
		if cs != "active" {
			item.CommitteePending = ""
		}
		if c.Query("overdue") != "1" || item.Overdue {
...
func reportRoundLabel(status, outcome string) string {
	if outcome != "" {
		return questionStatusLabel(outcome)
	}
	switch status {
	case "active":
