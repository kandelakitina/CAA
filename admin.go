...
func maintenanceAction(action string) (string, string, bool) {
	switch action {
	case "archive_all":
		return "Архивировать все материалы и отключить остальных пользователей", "В АРХИВ", true
	case "reset":
...
func (app *application) executeMaintenance(c *gin.Context) {
	...
	action := c.PostForm("action")
	_, phrase, ok := maintenanceAction(action)
	id, _ := strconv.ParseInt(c.PostForm("target_id"), 10, 64)
	...
	if action == "reset" {
		...
		if err == nil {
			_, err = tx.Exec(ctx, "TRUNCATE "+resetTables)
		}
		if err == nil {
			_, err = tx.Exec(ctx, "DELETE FROM sessions WHERE user_id<>$1", actor.ID)
		}
		if err == nil {
			_, err = tx.Exec(ctx, "DELETE FROM users WHERE id<>$1", actor.ID)
		}
	} else {
...
func archiveQuestions(ctx context.Context, tx pgx.Tx, actor user, id int64, all bool, reason string) error {
	for _, table := range []string{"internal_review_rounds", "committee_vote_rounds"} {
		if _, err := tx.Exec(ctx, "UPDATE "+table+" SET status='cancelled',cancelled_at=NOW() WHERE status='active' AND question_id IN (SELECT id FROM questions WHERE archived_at IS NULL AND ($1 OR id=$2))", all, id); err != nil {
			return err
		}
	}
...
func (app *application) processStorageCleanup(ctx context.Context) {
	...
	for i := 0; i < 25; i++ {
		...
		input := &s3.DeleteObjectInput{Bucket: &app.storage.bucket, Key: &key}
		if version != "" {
			input.VersionId = &version
		}
		callCtx, cancel := context.WithTimeout(ctx, 5*time.Second)
		_, deleteErr := app.storage.client.DeleteObject(callCtx, input)
		cancel()
...
func (app *application) retryStorageCleanup(c *gin.Context) {
	app.processStorageCleanup(c.Request.Context())
...
func (app *application) restoreUser(c *gin.Context) {
	id, ok := parsePositiveID(c, "id", "Некорректный пользователь")
	if !ok {
		return
	}
	...
	if err == nil {
		err = app.writeAudit(c.Request.Context(), tx, c.MustGet("user").(user), auditRecord{EventType: "user.restored", TargetType: "user", TargetID: &id, TargetLabel: name})
	}
	if err == nil {
...
func (app *application) revokeUserSessions(c *gin.Context) {
	...
	if err == nil {
		err = app.writeAudit(c.Request.Context(), tx, c.MustGet("user").(user), auditRecord{EventType: "user.sessions_revoked", TargetType: "user", TargetID: &id, TargetLabel: name})
	}
	if err == nil {
		err = tx.Commit(c.Request.Context())
	}
