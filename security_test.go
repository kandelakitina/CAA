...
func TestProtocolManagementRole(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, test := range []struct {
		role       string
		wantStatus int
		...
		t.Run(test.role, func(t *testing.T) {
			app := &application{}
			router := gin.New()
			router.POST("/protocols", func(c *gin.Context) {
				c.Set("user", user{Role: test.role})
			}, app.requireRole("secretary"), func(c *gin.Context) {
				c.Status(http.StatusNoContent)
			})
			response := httptest.NewRecorder()
			router.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/protocols", nil))
			if response.Code != test.wantStatus {
				t.Fatalf("status=%d, want %d", response.Code, test.wantStatus)
			}
		})
...
func TestInternalApproverRoles(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, test := range []struct {
		name       string
		user       user
		...
		{name: "assigned approver", user: user{Role: "approver", InternalService: "finance"}, wantStatus: http.StatusNoContent},
		{name: "unassigned approver", user: user{Role: "approver"}, wantStatus: http.StatusForbidden},
		{name: "secretary", user: user{Role: "secretary"}, wantStatus: http.StatusForbidden},
		{name: "committee", user: user{Role: "committee"}, wantStatus: http.StatusForbidden},
		{name: "observer", user: user{Role: "observer"}, wantStatus: http.StatusForbidden},
	} {
		t.Run(test.name, func(t *testing.T) {
			app := &application{}
			router := gin.New()
			router.POST("/questions/1/internal-review/respond", func(c *gin.Context) {
				c.Set("user", test.user)
			}, app.requireInternalApprover(), func(c *gin.Context) {
				c.Status(http.StatusNoContent)
			})
			response := httptest.NewRecorder()
			router.ServeHTTP(response, httptest.NewRequest(http.MethodPost, "/questions/1/internal-review/respond", nil))
			if response.Code != test.wantStatus {
				t.Fatalf("status=%d, want %d", response.Code, test.wantStatus)
			}
		})
